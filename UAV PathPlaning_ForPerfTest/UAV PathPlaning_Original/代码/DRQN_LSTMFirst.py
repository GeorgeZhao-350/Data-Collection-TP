...
class DRQNetWork(nn.Module):
    def __init__(self):
        ...
        self.LSTM_hidsize = 64

        self.fc1 = nn.Linear(NUM_STATES-9+self.LSTM_hidsize, 256)
        ...
        self.rnn=nn.LSTM(9,self.LSTM_hidsize)
    ...
    def forward(self, state):
        ...
        terminal_user, resident_state, _ = torch.hsplit(state, (9,40))

        shape = terminal_user.size()
        terminal_user = torch.reshape(terminal_user, [1, shape[1]])
        terminal_user,(self.h_unbatch,self.c_unbatch) = self.rnn(terminal_user,(self.h_unbatch,self.c_unbatch))
        torch.reshape(terminal_user,(1,-1))

        x=torch.cat((terminal_user,resident_state), 1)
    ...
    def batch_forward(self, batch_state):
        ...
        terminal_user, resident_state,_ = torch.hsplit(batch_state, (9,40))
        # state=torch.reshape(state, (state.shape[0], 1, state.shape[1]))

        shape = terminal_user.size()
        terminal_user = torch.reshape(terminal_user, [BATCH_SIZE, shape[1]])
        terminal_user, (self.h_unbatch, self.c_unbatch) = self.rnn(terminal_user, (self.h_unbatch, self.c_unbatch))
        torch.reshape(terminal_user, (1, -1))
...
class Replay_Memory:
    ...
    def store_transition(self, state, action, reward, next_state, terminal, td_err):
    ...
    def sample(self):
        ...
        index = np.random.choice(np.arange(max_mem), BATCH_SIZE, replace=False, p=probability[0:max_mem])
...
class DRQN():
    def __init__(self, shared_repbuffer):
        ...
        self.loss_func = nn.MSELoss(reduction="sum")  # mean square
    ...
    def store_transition(self, state, action, reward, next_state):
        index = self.replay_buffer.memory_counter % MEMORY_CAPACITY
        for i in range(NUM_STATES):
            self.replay_buffer.memory[index, i] = state[i]
        self.replay_buffer.memory[index, NUM_STATES] = action
        self.replay_buffer.memory[index, NUM_STATES + 1] = reward
        for i in range(NUM_STATES):
            self.replay_buffer.memory[index, NUM_STATES + 2 + i] = next_state[i]
        self.replay_buffer.memory_counter += 1
    ...
    def learn(self):
        ...
        sample_index = np.random.choice(
            self.replay_buffer.memory_counter if self.replay_buffer.memory_counter < MEMORY_CAPACITY else MEMORY_CAPACITY, BATCH_SIZE)
        batch_memory = self.replay_buffer.memory[sample_index, :]
...
if __name__ == '__main__':
    ...
    if perf_test:
        ...
        for i in range(3):
            ...
            # pathname_localnet = 'localnet_5_5_3_DQN_3cen_nTU_obv3.pth'
            pathname_localnet = 'localnetDRQN_Distributed'+ str(i) + '.pth'
