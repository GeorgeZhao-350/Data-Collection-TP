...
class DRQNetWork(nn.Module):
    def __init__(self):
        ...
        self.w_h = nn.Linear(self.LSTM_hidsize, 128, bias=False)
        self.g1 = nn.Linear(256, 128)
        self.g2 = nn.Linear(128, self.LSTM_hidsize)
    ...
    def forward(self, state):
        ...
        attention = nn.Softmax(dim=1)(self.g2(torch.tanh(self.g1(x) + self.w_h(self.h_unbatch))))
        x = torch.mul(x, attention)
    ...
    def batch_forward(self, batch_state):
        ...
        out = torch.zeros(1,BATCH_SIZE,NUM_ACTIONS)
        for i in range(TIME_LENGTH):
            # state=torch.reshape(state, (state.shape[0], 1, state.shape[1]))
            x = torch.relu(self.fc1(batch_state[i, :, :]))
            x = torch.relu(self.fc2(x))
            # x = torch.relu(self.fc3(x))
            shape = x.size()
            x = torch.reshape(x, (1, shape[0], shape[1]))
            attention = nn.Softmax(dim=1)(self.g2(torch.tanh(self.g1(x) + self.w_h(self.h_minibatch))))
            x = torch.mul(x, attention)
