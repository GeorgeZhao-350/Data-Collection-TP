...
class QNetWork(nn.Module):
    def __init__(self):
        ...
        self.A = nn.Linear(64, NUM_ACTIONS)
...
def train_multiuavs(file_name):
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                if action[i] != -1:
                    d3qn[i].store_transition(state[i], action[i], reward[i] - last_reward[i], next_state[i])
                    ep_reward[i] += reward[i]
                    last_reward[i] = reward[i]
...
def save_fig(file_name, reward_list, reward_test_avg, successrate, TUserverate, allsuccessrate, stepdone_avg):
    ...
    try:
        filepath="C:\DRQN_Data\D3QN_06092024.csv"
...
def application(test_model_name):
    d3qn_app = []
    for i in range(5):
        d3qn_app.append(D3QN())
        # pathname_localnet = 'localnet_5_5_3_DQN_3cen_nTU_obv3.pth'
        pathname_localnet = 'localnet' + test_model_name + str(i) + '.pth'
...
if __name__ == '__main__':
    ...
    filename = 'DDQN_28092024_R2_1_3UAV'
    ...
    if train_flag:
        ...
        if application_test:
            # env.n_uav = u
            # env.reset()
            test_model_name = 'D3QN_12092024_R2_1_5UAV' # 加载需测试的模型；训练完成的模型：‘DQN_distributed'
            app(test_model_name)

    if perf_test:
        num_test = 500
        seed_nums = [80, 90, 95]
        test_var = range(3, 4, 1)
        # d3qn_app = D3QN()
        # # pathname_localnet = 'localnet_5_5_3_DQN_3cen_nTU_obv3.pth'
        ...
        for i in range(3):
            ...
            # pathname_localnet = 'localnet' + filename + '0' + '.pth'
            d3qn_app[i].load_net(pathname_localnet)
        ...
        for i, var in enumerate(test_var):
            for n, seed in enumerate(seed_nums):
                print('test performance for variable {}\n'.format(var))
                _, _, successrate, TUserverate, step_done, avgtime, collision_rate, timeout_rate = performance_test_multiuavs(
                    d3qn_app, num_test, n_UAV=3, n_TU=100, n_dyob=10, n_ob=10, seed_num=seed)
        ...
        p = 'A_Result\DDQN_OB10_DOB10_TU100_Small'  #  D3QN_28092024_T2_TU_18_22
