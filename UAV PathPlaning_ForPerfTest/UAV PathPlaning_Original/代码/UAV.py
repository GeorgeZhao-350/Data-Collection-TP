...
class UAV():
    def __init__(self, env, id, x, y, tar):
        ...
        self.p_move = 1
        self.step_done = 0
        self.energy = int(6 / self.stepWay)
        self.x_tar_f = tar[0]
        self.y_tar_f = tar[1]
        self.x_tar = tar[0]
        self.y_tar = tar[1]
    ...
    def update(self, action, hover):
        ...
        if self.stage >= self.segmentation:
            ...
            self.x_tar = (self.id+1-0.5)/self.env.n_uav
            self.y_tar = self.stage/self.segmentation
