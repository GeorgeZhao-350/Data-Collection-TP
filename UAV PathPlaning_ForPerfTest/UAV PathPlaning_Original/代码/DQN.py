...
BATCH_SIZE = 10    #单次学习取记录数 # default 128
EPS_START = 0.8
EPS_END = 0.1
EPS_DECAY = 5000
...
def save_fig(file_name, reward_list, reward_test_avg, successrate, TUserverate, allsuccessrate, stepdone_avg):
    ...
    try:
        filepath="C:\DRQN_Data\DQN_forReward2.csv"
...
def performance_test_distributed(network, num_test, n_UAV=3, n_TU=15, n_dyob=5, n_ob=5, seed_num=95):
    num_success = 0
    num_allsuccess = 0
    num_TUserved = 0
    reward_test = 0
    ...
    # collision_rate = num_collision / num_test
...
if __name__ == '__main__':
    ...
    if train_flag:
        ...
        # train('_5_5_3_DQN_connectivity_3cen_nTU_obv3')
        train_distributed('DQN_forReward2')
    ...
    if perf_test:
        ...
        for i in range(3):
            d3qn_app.append(DQN())
            # pathname_localnet = 'localnet_5_5_3_DQN_3cen_nTU_obv3.pth'
            pathname_localnet = 'localnetDQN_distributed'+ str(i) + '.pth'
            d3qn_app[i].load_net(pathname_localnet)
        ...
        # Total_severate = np.zeros((len(seed_nums), len(test_var)))
        ...
        for i, var in enumerate(test_var):
            for n, seed in enumerate(seed_nums):
                ...
                _, _, successrate, TUserverate, step_done, avgtime, collision_rate, timeout_rate = performance_test_distributed(
