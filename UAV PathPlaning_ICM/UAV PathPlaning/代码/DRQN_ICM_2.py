...
class ICM_Model(nn.Module):
    ...
    def forward(self, fea_state, action):
        fwd = torch.relu(self.fwd_fc1(torch.cat((fea_state, action), dim=len(fea_state.shape)-1)))
...
class ICM():
    ...
    def update(self, batch_state, batch_next_state, batch_action):
        ...
        inv_loss = self.loss_func_inv(inv, batch_action.long().flatten())
        self.model.zero_grad()
        inv_loss.backward()
        with torch.no_grad():
            self.optimizer.step()
...
class DRQN():
    ...
    def learn(self, icm_moduel):
        ...
        q_target = batch_reward + GAMMA * q_next.max(1)[0].view(BATCH_SIZE, 1)*batch_terminal
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                ...
                if d3qn[i].memory_counter >= TRAIN_THRESHOLD and step%5 == 0:
                    d3qn[i].learn(icm[i])
...
def application(modelname):
    ...
    for i in range(env.n_uav):
        ...
        plt.plot(uav_x[i], uav_y[i], color=env.colorbar[i,:])
    # for i in range(env.n_ob_dynamic):
