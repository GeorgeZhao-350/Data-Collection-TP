...
BATCH_SIZE = 4    #单次学习取记录数 # default 128
EPS_START = 0.8
EPS_END = 0.1
EPS_DECAY = 5000
...
class DRQNetWork(nn.Module):
    def __init__(self):
        ...
        self.rnn=nn.LSTM(NUM_STATES,self.LSTM_hidsize)
        self.h_minibatch = torch.zeros(1,15,self.LSTM_hidsize)
        self.c_minibatch = torch.zeros(1,15,self.LSTM_hidsize)

    def forward(self, state):
        # state=torch.reshape(state, (state.shape[0], 1, state.shape[1]))

        shape=state.size()
        x=torch.reshape(state,[1,shape[1]])
        ...
        out = torch.reshape(self.out(x),(1,-1))
        #out = torch.tanh(self.out(x))

        return out

    def td_forward(self, state):
        # state=torch.reshape(state, (state.shape[0], 1, state.shape[1]))
        h_tdbatch = torch.zeros(1, self.LSTM_hidsize)
        c_tdbatch = torch.zeros(1, self.LSTM_hidsize)
        ...
        with torch.no_grad():
            ...
            x,(h_tdbatch,c_tdbatch) = self.rnn(x,(h_tdbatch,c_tdbatch))
            #x = torch.relu(self.fc4(x))
    ...
    def batch_forward(self, batch_state):
        ...
        # state=torch.reshape(state, (state.shape[0], 1, state.shape[1]))
        # shape = x.size()
        # x = torch.reshape(x, [shape[0],shape[2], shape[1]])
        x,(self.h_minibatch,self.c_minibatch) = self.rnn(batch_state,(self.h_minibatch,self.c_minibatch))
        x=torch.reshape(x,(-1, self.LSTM_hidsize))
        x = torch.relu(self.fc1(x))
        x = torch.relu(self.fc2(x))
        # x = torch.relu(self.fc3(x))
        #x = torch.relu(self.fc4(x))
        ...
        out = self.out(x)
        # out = torch.reshape(self.out(x),(env.uavs,-1))
        #out = torch.tanh(self.out(x))
...
class Replay_Memory:
    def __init__(self):
        ...
        self.memory = np.zeros((MEMORY_CAPACITY, env.n_uav, NUM_STATES * 2 + 2+1))
        self.priorities = np.zeros(MEMORY_CAPACITY)
        self.td_err = np.zeros((MEMORY_CAPACITY, 1))
        self.beta = 0.4

    def store_transition(self, state, action, reward, next_state, terminal, uav_id):
        index = self.memory_counter % MEMORY_CAPACITY
        for i in range(NUM_STATES):
            self.memory[index, uav_id, i] = state[i]
        self.memory[index, uav_id, NUM_STATES] = action
        self.memory[index, uav_id, NUM_STATES + 1] = reward
        for i in range(NUM_STATES):
            self.memory[index, uav_id, NUM_STATES + 2 + i] = next_state[i]
        self.memory[index, uav_id, 2*NUM_STATES + 2] = terminal
        self.memory_counter += 1
        return self.memory[index, uav_id, :]
    ...
    def sample(self):
        ...
        index = np.random.choice(np.arange(max_mem), 16, replace=False, p=probability[0:max_mem])
        batch_memory = np.zeros((BATCH_SIZE, env.n_uav, NUM_STATES * 2 + 2+1))
        batch_cnt=0
        while batch_cnt<16:
            for n in range(env.n_uav):
                if index[batch_cnt]<self.memory_counter and math.fabs(index[batch_cnt]-self.memory_counter).__int__() <= BATCH_SIZE:
                    index[batch_cnt]=self.memory_counter-BATCH_SIZE-1
                for i in range(index[batch_cnt], index[batch_cnt]+BATCH_SIZE):
                    if self.memory[index[batch_cnt], n, 2*NUM_STATES + 2]==1:
                        index[n]=i-BATCH_SIZE
                        break
                if index[batch_cnt]<0:
                    index[batch_cnt]=0
                elif index[batch_cnt]+BATCH_SIZE>=max_mem:
                    index[batch_cnt]=max_mem-BATCH_SIZE-1
                # imp = self.prob_imp(probability[index], self.beta)
                batch_memory[:, batch_cnt, :] = self.memory[index[batch_cnt]:index[batch_cnt]+BATCH_SIZE, n, :]
                batch_cnt+=1
                if batch_cnt>=16:
                    break
    ...
    def sample_random(self):
        ...
        max_mem = min(self.memory_counter, MEMORY_CAPACITY)
        index_array = np.random.choice(np.arange(max_mem), 15, replace=False)
        index = index_array
        batch_memory = np.zeros((BATCH_SIZE, 15, NUM_STATES * 2 + 2 + 1))
        batch_cnt = 0
        while batch_cnt < 15:
            for n in range(env.n_uav):
                if index[batch_cnt] < self.memory_counter and math.fabs(
                        index[batch_cnt] - self.memory_counter).__int__() <= BATCH_SIZE:
                    index[batch_cnt] = self.memory_counter - BATCH_SIZE - 1
                for i in range(index[batch_cnt], index[batch_cnt] + BATCH_SIZE):
                    if self.memory[index[batch_cnt], n, 2 * NUM_STATES + 2] == 1:
                        index[n] = i - BATCH_SIZE
...
class DRQN():
    ...
    def tderr_calc(self, state, action, reward, next_state, terminal): # size of menory array: (1,-1)
        with torch.no_grad():
            ...
            q_local = self.local_net.td_forward(batch_state).gather(1, batch_action)

            # DQN
            # q_next = self.target_net(batch_next_state).detach()

            # DDQN
            local_action_value = self.local_net.td_forward(batch_next_state).detach()
            ...
            q_next = self.target_net.td_forward(batch_next_state).gather(1, local_action).detach()
    ...
    def learn(self):
        ...
        batch_memory, sample_index=self.replay_buffer.sample_random()
        ...
        q_local = self.local_net.batch_forward(batch_state).gather(1, torch.reshape(batch_action,(-1,1)))
        ...
        #这里有问题，如果next state为中止状态，q值应为batch_reward?
        q_target = batch_reward + GAMMA * q_next.max(1)[0].view(15*BATCH_SIZE, 1)*(batch_terminal.to(dtype=int))
...
def train_distributed(file_name):
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                if action[i] != -1:
                    exp_memory.store_transition(state[i], action[i], reward[i], next_state[i], done[i], i)
...
if __name__ == '__main__':
    ...
    if perf_test:
        ...
        for i in range(3):
            ...
            # pathname_localnet = 'localnet_5_5_3_DQN_3cen_nTU_obv3.pth'
            pathname_localnet = 'localnetDRQN_SEPER2'+ str(i) + '.pth'
