...
def train(file_name):
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                if action[i] != -1:
                    exp_buffer.store_transition(state[i], action[i], reward[i], next_state[i])
                    ep_reward += reward[i]
...
def train_multiuavs(file_name):
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                if action[i] != -1:
                    td_err[i]=d3qn[i].tderr_calc(exp_buffer.store_transition(state[i], action[i], reward[i], next_state[i]))
...
if __name__ == '__main__':
    ...
    filename = 'miu_ED3QN_Multiuavs_5000-32_32'
