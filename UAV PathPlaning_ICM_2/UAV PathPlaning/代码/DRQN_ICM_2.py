...
class ICM_Model(nn.Module):
    def __init__(self):
        ...
        self.feamap_size = 512

        self.fwd_fc1 = nn.Linear(self.feamap_size+1, self.feamap_size*2).cuda()
    ...
    def inverse(self,fea_state, fea_next_state):
        ...
        pre_action = torch.relu(self.inv_fc3(pre_action))

        return pre_action

    def forward(self, fea_state, action):
        fwd = torch.relu(self.fwd_fc1(torch.cat((fea_state, action/(NUM_ACTIONS-1)), dim=len(fea_state.shape)-1)))
...
class ICM():
    ...
    def update(self, batch_state, batch_next_state, batch_action):
        ...
        inv = torch.softmax(self.model.inverse(f_state_inv, f_next_state_inv), dim=len(f_state_inv.shape)-1)
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                ...
                if d3qn.memory_counter >= TRAIN_THRESHOLD and step%100 == 0:
                    d3qn.learn()
                step += 1
...
def train_distributed(file_name):
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            # if len(curiosity) > 51:
            #     for i in range(50):
            #         temp_curiosity += curiosity[len(curiosity) - 1 - i]
            #     avg_curiosity = temp_curiosity / 50
            for i in range(env.n_uav):
                if action[i] != -1:
                    icm_reward = 0
                    if d3qn[i].memory_counter >= TRAIN_THRESHOLD:
                        icm_reward = math.sqrt(icm[i].predit(state[i], next_state[i], action[i]))
                        # add_reward = max(avg_curiosity / 2, icm_reward) - avg_curiosity / 2
...
def application(modelname):
    ...
    while 1:
        ...
        for i in range(env.n_uav):
            if action[i] != -1:
                ...
                uav_x_inv[i].append((1-env.uavs[i].x) * env.length)
                uav_y_inv[i].append((1 - env.uavs[i].y) * env.length)
    ...
    for i in range(env.n_uav):
        if env.uavs[i].d_tar < (2 * env.uavs[i].stepWay):
            ...
            uav_x_inv[i].append((1-env.uavs[i].x_tar) * env.length)
            uav_y_inv[i].append((1 - env.uavs[i].y_tar) * env.length)
            uav_y[i].append(env.uavs[i].y_tar * env.width)
            uav_z[i].append(env.uavs[i].H * env.length)
        plt.plot(uav_x[i], uav_y[i])
    for i in range(env.n_ob_dynamic):
        plt.plot(ob_x[i], ob_y[i], 'r-.')
    env.render_3D(uav_y, uav_x_inv, uav_z)
    plt.pause(5)
...
if __name__ == '__main__':
    ...
    if perf_test:
        ...
        for i in range(6, 25, 3):
            ...
            _, _, successrate, TUserverate, step_done, avgtime, collision_rate = performance_test_distributed(d3qn_app, num_test, n_TU=i, n_dyob=5)
        ...
        for i in range(3, 8, 1):
            print('test performance for {} dynamic obstacles'.format(i))
            _, _, successrate, TUserverate, step_done, avgtime, collision_rate = performance_test_distributed(d3qn_app, num_test, n_TU=15, n_dyob=i)
            UAV_successrate.append(successrate)
            UAV_TUserverate.append(TUserverate)
            UAV_step_done.append(step_done)
            UAV_time_avg.append(avgtime)
