...
def draw_cylinder(x, y, h, r, resolution, ax):
    ...
    ax.plot_surface(X, Y, Z, rstride=5, cstride=5, color='k', alpha=0.5)
    ...
    ax.plot_trisurf(X_disc, Y_disc, [z_top] * resolution, color='k', alpha=0.5)  # Top surface
    ax.plot_trisurf(X_disc, Y_disc, [z_bottom] * resolution, color='k', alpha=0.5)  # Bottom surface
...
class ENV():
    ...
    def render(self, flag = 0):
        if flag:
            ...
            #目标位置
            for i in range(self.n_uav):
                plt.scatter(self.target[i][0] * self.length, self.target[i][1] * self.width, s=20, color='r', marker='x')
            #固定障碍物
            for ob in self.obs:
        ...
        for tu in self.tus:
            ...
            plt.scatter(tu.x * self.length, tu.y * self.width, s=30, color='green' if tu.flag_done else 'brown', marker='*')
            theta = np.arange(0, 2 * np.pi, 0.01)
            x = tu.x * self.length + tu.r * self.length * np.cos(theta)
            y = tu.y * self.width + tu.r * self.length * np.sin(theta)
            plt.plot(x, y,linewidth=1 , color='green' if tu.flag_done else 'brown')
            plt.text((tu.x+0.01)*self.length, (tu.y+0.01)*self.width, '%.2f'%tu.I)

        #无人机
        for uav in self.uavs:
            plt.scatter(uav.x * self.length, uav.y * self.width, s=40, color='b', marker='^')
        plt.pause(0.01)
