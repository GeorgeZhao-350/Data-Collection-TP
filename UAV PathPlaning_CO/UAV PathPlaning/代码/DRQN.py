...
class DRQN():
    def __init__(self):
        ...
        self.loss_func = nn.HuberLoss()  # mean square
    ...
    def choose_action_train(self, state, state_s1, state_s2, step_done):
        ...
        state_s1 = torch.unsqueeze(torch.FloatTensor(state_s1), 0)  # get a 1D array
        state_s2 = torch.unsqueeze(torch.FloatTensor(state_s2), 0)  # get a 1D array
    ...
    def choose_action_apply(self, state, state_s1, state_s2):
        ...
        state_s1 = torch.unsqueeze(torch.FloatTensor(state_s1), 0)
        state_s2 = torch.unsqueeze(torch.FloatTensor(state_s2), 0)
        with torch.no_grad():
            action_value = self.local_net.forward(state, state_s1, state_s2).detach()
        action_value = torch.reshape(action_value, (1, -1))
        action = int(torch.max(action_value, 1)[1].data.numpy())
    ...
    def sample(self):
        ...
        batch_state_s = torch.zeros((TIME_LENGTH, BATCH_SIZE, NUM_NEIGH_STATES))
        batch_next_state_s = torch.zeros((TIME_LENGTH, BATCH_SIZE, NUM_NEIGH_STATES))
...
if __name__ == '__main__':
    ...
    if perf_test:
        ...
        for i in range(9, 22, 3):
            ...
            _, _, successrate, TUserverate, step_done, avgtime, collision_rate, equality = performance_test_distributed(d3qn_app, num_test, n_TU=i, n_dyob=5)
