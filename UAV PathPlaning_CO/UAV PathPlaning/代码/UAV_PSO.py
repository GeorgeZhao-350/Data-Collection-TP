...
class UAV():
    ...
    def get_tu_can(self):
        ...
        if self.env.tu_service_state[self.id]<len(self.env.tu_allocation[self.id]):
            tu=self.env.tus[self.env.tu_allocation[self.id][self.env.tu_service_state[self.id]]]
    ...
    def state(self):
        ...
        tu = self.env.tus[self.env.tu_allocation[self.id][self.env.tu_service_state[self.id]]]
        dir_tu_before = math.atan2(tu.y - self.y, tu.x - self.x)
        #方向角归一化
    ...
    #     rate_tu = self.B * math.log(1 + S , 2) #香农公式
        ...
        state_grid.append(d / self.senser_tu_r)
        state_grid.append(tu.I / self.env.maxI)
