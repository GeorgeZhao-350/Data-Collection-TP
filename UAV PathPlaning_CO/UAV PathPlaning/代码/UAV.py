...
class UAV():
    def __init__(self, env, id, x, y, tar):
        ...
        #无人机之间通信的路劲损耗阈值
        self.h_free_up = 1
        self.iot_service_cnt = 0
        self.iot_service_rwd = 0
    ...
    def state(self):
        ...
        for uav in self.env.uavs:
            if uav.id != self.id:
                if abs(self.x - uav.x) > self.senser_ob_r or abs(self.y - uav.y) > self.senser_ob_r or uav.done == True:
                    continue
                d = math.sqrt((uav.x - self.x) ** 2 + (uav.y - self.y) ** 2)
                if d < self.senser_uav_r:
                    d_uav.append(d)
                    id_uav.append(uav.id)
                    n_uav += 1
        i_uav = sorted(range(len(d_uav)), key=lambda k: d_uav[k]) #对感知范围内其他无人机的距离进行排序，以获取最近的两架无人机的索引
        for i in range(2):
            id_near_uav.append(id_uav[i_uav[i]])
    ...
    def update(self, action, hover):
        ...
        #     uav_service_cnt.append(uav.iot_service_cnt)
        # if denominator != 0:
        #     fairness = numerator ** 2 / denominator / self.env.n_uav * (1 - self.iot_service_cnt/max(uav_service_cnt)) + 0.5
        # else:
        #     fairness = 0
        ...
        for i in range(self.num_tu_can):
            ...
            # for uav in self.env.uavs:
            #     for tu_near in uav.tu_can[0 : uav.num_tu_can]:
            #         if self.tu_can[self.id_nearlist[i]].id == tu_near.id:
            #             weight *= fairness
        ...
        for uav in self.env.uavs:
            if uav.id != self.id and uav.done == False:
                d_uav.append(math.sqrt((uav.x - self.x)**2 + (uav.y - self.y)**2))
                #自由空间的路径损耗
                h_free.append(20 * math.log(4 * math.pi * self.fc * d_uav[len(d_uav)-1] / self.c , 2))
        d_uav.sort()
        h_free.sort()
        if len(d_uav) >= 2:
            for i in range(2):
                if d_uav[i] < 0.05:
                    reward -= 3
                elif d_uav[i] < 0.05 + 2 * self.stepWay and d_uav[i] >= 0.05:
                    reward -= 2
                elif h_free[i] > self.h_free_up:
                    reward -= 0.05
        elif len(d_uav) == 1:
            if d_uav[0] < 0.05:
                reward -= 3
            elif d_uav[0] < 0.05 + 2 * self.stepWay and d_uav[0] >= 0.05:
                reward -= 2
            elif h_free[0] > self.h_free_up:
                reward -= 0.05

        if d_tar_next < (2 * self.stepWay):
            if self.stage >= self.segmentation:
                self.done = True

        if self.y >= self.y_tar - 3 * self.stepWay and flag_tu:
            self.stage += 1

        reward += reward_tar + reward_tu + reward_ob
        self.iot_service_rwd = reward_tu
        ...
        if self.stage >= self.segmentation:
            self.x_tar = self.x_tar_f
            self.y_tar = self.y_tar_f
        else:
            self.x_tar = (self.id+1-0.5)*self.env.x_bound/self.env.n_uav
            self.y_tar = self.stage/self.segmentation*self.env.y_bound
