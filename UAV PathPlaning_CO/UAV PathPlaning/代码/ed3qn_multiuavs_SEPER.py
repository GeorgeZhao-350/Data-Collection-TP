...
class D3QN():
    ...
    def tderr_calc(self, state, action, reward, next_state, terminal): # size of menory array: (1,-1)
        ...
        batch_terminal=torch.reshape(torch.logical_not(torch.BoolTensor(np.array(terminal,dtype=np.bool8))),(1,-1))
    ...
    def learn(self):
        ...
        # td_next = self.local_net(batch_next_state).gather(1, local_action).detach()
        # td_target = batch_reward + GAMMA * td_next.max(1)[0].view(BATCH_SIZE, 1)
        #
        # # i = np.arange(BATCH_SIZE)
        # diff = torch.abs(td_target - q_local)
        for i in range(BATCH_SIZE):
            idx = sample_index[i]
...
def train(file_name):
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                if action[i] != -1:
                    exp_buffer.store_transition(state[i], action[i], reward[i], next_state[i], terminal[i], td_err=0)
                    ep_reward += reward[i]
                if terminal[i]==False:
                    done=False
...
def train_multiuavs(file_name):
    ...
    for _ in enumerate(tqdm(range(episodes))):
        ...
        while 1:
            ...
            for i in range(env.n_uav):
                if action[i] != -1:
                    td_err[i]=d3qn[i].tderr_calc(state[i], action[i], reward[i], next_state[i], terminal[i])
                    exp_buffer.store_transition(state[i], action[i], reward[i], next_state[i], terminal[i], td_err[i])
...
if __name__ == '__main__':
    ...
    filename = 'miu_ED3QN_Multiuavs_5000-32_32_SEPER'
